...
namespace bl
{
    namespace messaging
    {
        ...
        {
            bl::eh::void_exception_callback_t m_defaultCallback = bl::dm::ServerErrorHelpers::defaultEhCallback();

        public:

            static auto getServerErrorAsGraphQL(
                SAA_in      const std::exception_ptr&                       eptr,
                SAA_in_opt  const bl::eh::void_exception_callback_t&        callback = m_defaultCallback
                )
                -> std::string
            {
                return bl::dm::DataModelUtils::getDocAsPrettyJsonString(
                    createServerErrorGraphQLObject( eptr, callback )
                    );
            }
        ...
        private:
            ...
            static auto createServerErrorGraphQLObject(
                SAA_in      const std::exception_ptr&                       eptr,
                SAA_in      const bl::eh::void_exception_callback_t&        callback
                )
                -> om::ObjPtr< bl::dm::ServerErrorGraphQL >
            {
                ...
                const auto errorCode =
                    static_cast< bl::eh::errc::errc_t >( error -> exceptionProperties() -> errorCode() );

                const auto expected =
                    BrokerErrorCodes::tryGetExpectedErrorMessage( bl::eh::errc::make_error_code( errorCode ) );

                const auto message = expected.empty() ? error -> message() : expected;

                result -> message( message + " [error code " + std::to_string( errorCode ) + "]" );
